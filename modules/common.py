"""
Talent Management Platform - Common Modules
...
# ================= COMPREHENSIVE REPORT DATA BUILDER =================
def build_full_user_report_sections(user, limit_per_section=12):
    """
    Pulls data from every feature/table across the platform (assessments, quizzes,
    mock interviews, coding practice, resumes/ATS, saved internships/placements,
    learning progress, goals, certificates, achievements) and returns a dict of
    section_name -> list[str], ready to feed into a PDF report generator.
    """
    uid = user["id"]

    assessments = run_query("SELECT * FROM assessments WHERE user_id=? ORDER BY created_at DESC LIMIT ?",
                             (uid, limit_per_section), fetch=True)
    quizzes = run_query("SELECT * FROM quiz_results WHERE user_id=? ORDER BY created_at DESC LIMIT ?",
                         (uid, limit_per_section), fetch=True)
    interviews = run_query("SELECT * FROM mock_interview_results WHERE user_id=? ORDER BY created_at DESC LIMIT ?",
                            (uid, limit_per_section), fetch=True)
    coding = run_query("SELECT * FROM coding_practice WHERE user_id=? ORDER BY created_at DESC LIMIT ?",
                        (uid, limit_per_section), fetch=True)
    resumes = run_query("SELECT * FROM resume_data WHERE user_id=? ORDER BY created_at DESC LIMIT ?",
                         (uid, limit_per_section), fetch=True)
    projects = run_query("SELECT * FROM projects WHERE user_id=? ORDER BY created_at DESC LIMIT ?",
                          (uid, limit_per_section), fetch=True)
    progress_rows = run_query("SELECT * FROM learning_progress WHERE user_id=?", (uid,), fetch=True)
    goals = run_query("SELECT * FROM goals WHERE user_id=?", (uid,), fetch=True)
    certs = run_query("SELECT * FROM certificates WHERE user_id=?", (uid,), fetch=True)
    achievements = run_query("SELECT * FROM achievements WHERE user_id=?", (uid,), fetch=True)

    placements = [p for p in projects if p["title"].startswith("[Placement]")]
    internships = [p for p in projects if p["title"].startswith("[Internship]")]
    other_projects = [p for p in projects if not p["title"].startswith(("[Placement]", "[Internship]"))]
    ...
    avg_quiz_score = round(sum((q["score"] / q["total"] * 100) if q["total"] else 0 for q in quizzes) / len(quizzes), 1) if quizzes else 0
    avg_interview_score = round(sum(i["score"] for i in interviews) / len(interviews), 1) if interviews else 0
    latest_ats = resumes[0]["ats_score"] if resumes else 0

    sections = {}

    sections["Performance Snapshot"] = [
        f"Learning modules started: {total_tasks}  |  Completed: {done_tasks}  |  Avg. progress: {avg_progress}%",
        f"Quizzes attempted: {len(quizzes)}  |  Avg. quiz score: {avg_quiz_score}%",
        f"Mock interviews completed: {len(interviews)}  |  Avg. interview score: {avg_interview_score}/10",
        f"Coding problems attempted: {len(coding)}",
        f"Latest resume ATS score: {latest_ats}/100" if resumes else "No resume analyzed yet.",
        f"Certificates earned: {len(certs)}  |  Active goals: {len(goals)}  |  Achievements: {len(achievements)}",
    ]

    sections["Assessment History"] = (
        [f"{a['assessment_type']} — Result: {a['result']}  (Score: {a['score']})  on {a['created_at'][:10]}" for a in assessments]
        or ["No assessments completed yet."])

    sections["Quiz Results"] = (
        [f"{q['quiz_topic']} — {q['score']}/{q['total']}  on {q['created_at'][:10]}" for q in quizzes]
        or ["No quizzes attempted yet."])

    sections["Mock Interview Performance"] = (
        [f"[{i['role']}] {i['question'][:70]}{'...' if len(i['question']) > 70 else ''} — Score: {i['score']}/10  on {i['created_at'][:10]}" for i in interviews]
        or ["No mock interviews completed yet."])

    sections["Coding Practice Submissions"] = (
        [f"{c['problem']} ({c['language']}) — {c['status']}  on {c['created_at'][:10]}" for c in coding]
        or ["No coding submissions yet."])

    sections["Resume / ATS Analysis History"] = (
        [f"Target Role: {r['target_role'] or 'General'} — ATS Score: {r['ats_score']}/100  on {r['created_at'][:10]}" for r in resumes]
        or ["No resume analysis performed yet."])

    if placements:
        sections["Placement Applications Tracked"] = [
            f"{p['title'].replace('[Placement] ', '')} — {p['description']} — Status: {p['status']}" for p in placements]
    if internships:
        sections["Internships Saved"] = [
            f"{p['title'].replace('[Internship] ', '')} — {p['description']} — Status: {p['status']}" for p in internships]
    if other_projects:
        sections["Projects"] = [f"{p['title']} — {p['status']}" for p in other_projects]

    sections["Learning Progress & Enrolled Courses"] = (
        [f"{p['module']} — {p['task']} — {p['status']} ({p['progress_pct']}%)" for p in progress_rows]
        or ["No learning modules started yet."])

    sections["Goals"] = (
        [f"{g['goal_text']} — {g['status']} (Target: {g['target_date'] or 'N/A'})" for g in goals]
        or ["No goals set yet."])

    sections["Certificates Earned"] = (
        [f"{c['title']} — Issued {c['issued_on'][:10]} — Code: {c['cert_code']}" for c in certs]
        or ["No certificates earned yet."])

    if achievements:
        sections["Achievements & Badges"] = [f"{a['title']} — {a['badge']} — {a['earned_on'][:10]}" for a in achievements]
...
def page_profile(user):
    ...
    st.caption("Generate a comprehensive PDF snapshot of your entire journey — profile, assessments, quizzes, mock interviews, coding practice, resumes, internships/placements, courses, goals, and certificates — useful to verify your progress at a later date (e.g. for mentors, parents, recruiters, or your own records).")
    if st.button("📄 Generate Verification Report", type="primary"):
        extra = get_profile_extra(user)
        sections = build_full_user_report_sections(user)

        verification_code = f"TMP-VER-{uuid.uuid4().hex[:8].upper()}"
...
def page_progress_tracking(user):
    ...
    if st.button("📄 Generate Weekly Progress Report (PDF)"):
        ...
        extra = get_profile_extra(user)
        full_sections = build_full_user_report_sections(user)
        sections = {"AI Weekly Summary": report_text.split("\n")}
        sections.update(full_sections)

        verification_code = f"TMP-WK-{uuid.uuid4().hex[:8].upper()}"
        pdf_bytes = generate_verification_report(user, extra, sections, verification_code)
        run_query("INSERT INTO reports (user_id, report_type, content, created_at) VALUES (?,?,?,?)",
