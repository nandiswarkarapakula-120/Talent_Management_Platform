"""
Talent Management Platform - High School Student Modules
...
def _profile_keywords(user):
    """Builds a lowercase keyword list from the student's own profile selections."""
    extra = get_profile_extra(user)
    raw = [extra.get("interest_area", "")] + extra.get("favorite_subjects", "").split(",") + [extra.get("target_exam", ""), extra.get("board", "")]
    return [k.strip().lower() for k in raw if k and k.strip() and k.strip().lower() != "not sure yet"], extra


# ================= CAREER EXPLORER =================
def page_career_explorer(user):
    hero("Career Explorer", "Discover exciting career paths and what they involve.", "🧭")
    keywords, extra = _profile_keywords(user)
    ...
    def is_relevant(p):
        text = f"{p['title']} {p['description']} {p['required_skills']}".lower()
        return any(k in text for k in keywords) if keywords else False

    if not search:
        filtered = sorted(filtered, key=lambda p: not is_relevant(p))
        if keywords:
            st.caption(f"✨ Ranked using your profile: {', '.join(keywords[:5])}")
        else:
            st.info("💡 Fill in your subjects & interest area in My Profile to see careers ranked just for you!")
...
INTEREST_TO_FIELD = {
    "Science & Engineering": "Software Engineer",
    "Medicine & Healthcare": "Data Scientist",
    "Commerce & Finance": "Cloud Architect",
    "Arts & Design": "UI/UX Designer",
    "Humanities & Civil Services": "Cybersecurity Analyst",
...
def page_future_skills_roadmap(user):
    ...
    extra = get_profile_extra(user)
    fields = ["Software Engineer", "Data Scientist", "UI/UX Designer", "Cybersecurity Analyst", "Cloud Architect"]
    default_field = INTEREST_TO_FIELD.get(extra.get("interest_area", ""), fields[0])
    if extra.get("interest_area") and extra.get("interest_area") != "Not Sure Yet":
        st.caption(f"✨ Suggested based on your interest area ({extra['interest_area']}): **{default_field}**")
    goal = st.selectbox("What field interests you most?", fields, index=fields.index(default_field))
...
def page_daily_tasks(user):
    ...
    extra = get_profile_extra(user)
    favorite_subjects = [s.strip() for s in extra.get("favorite_subjects", "").split(",") if s.strip()]
    subject_for_today = favorite_subjects[0] if favorite_subjects else "a subject you enjoy"

    today = datetime.now().strftime("%Y-%m-%d")
    random.seed(today + str(user["id"]))
    task_pool = DAILY_TASKS + [
        f"Read one article or watch a video about {subject_for_today} (15 min)",
        f"Solve 3 practice problems related to {subject_for_today}",
    ]
    todays_tasks = random.sample(task_pool, 4)

    section_title(f"Today's Tasks — {datetime.now().strftime('%d %B %Y')}")
    if favorite_subjects:
        st.caption(f"✨ Personalized using your favorite subjects: {', '.join(favorite_subjects)}")
    completed_count = 0
...
def page_aptitude_practice(user):
    hero("Aptitude Practice", "Sharpen your logical, quantitative & subject reasoning.", "🔢")
    extra = get_profile_extra(user)
    favorite_subjects = [s.strip() for s in extra.get("favorite_subjects", "").split(",") if s.strip()]

    subject_options = ["Aptitude (General)", "Math", "Science", "English", "Social Studies"]
    default_subject = next((f"{s}" for s in favorite_subjects if s in ["Math", "Science", "English", "Social Studies"]), "Aptitude (General)")
    if favorite_subjects:
        st.caption(f"✨ Based on your favorite subjects ({', '.join(favorite_subjects)}), we've pre-selected a relevant focus below.")
    subject_choice = st.selectbox("Choose your practice focus", subject_options,
                                   index=subject_options.index(default_subject) if default_subject in subject_options else 0)
    topic_key = "Aptitude" if subject_choice == "Aptitude (General)" else subject_choice

    if "apt_quiz" not in st.session_state or st.session_state.get("apt_topic_active") != topic_key:
        st.session_state.apt_quiz = generate_quiz(topic_key, 5)
        st.session_state.apt_topic_active = topic_key
    ...
    with st.form("aptitude_quiz"):
        ...
        if st.form_submit_button("Submit", type="primary"):
            ...
            run_query("INSERT INTO quiz_results (user_id, quiz_topic, score, total, details, created_at) VALUES (?,?,?,?,?,?)",
                       (user["id"], topic_key, score, len(st.session_state.apt_quiz), "", now()))
...
def page_communication_skills(user):
    ...
    with tab1:
        ...
        if favorite_subjects:
            prompts.append(f"Explain why you enjoy {favorite_subjects[0]} and how it connects to your future goals.")
