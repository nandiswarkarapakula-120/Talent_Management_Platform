"""
Talent Management Platform - Admin Portal
...
def page_generate_reports(admin):
    ...
    active_users = run_query("SELECT COUNT(*) c FROM users WHERE is_active=1", fetchone=True)["c"]
    total_courses = run_query("SELECT COUNT(*) c FROM courses", fetchone=True)["c"]
    total_career_paths = run_query("SELECT COUNT(*) c FROM career_paths", fetchone=True)["c"]
    total_certs = run_query("SELECT COUNT(*) c FROM certificates", fetchone=True)["c"]
    quiz_count = run_query("SELECT COUNT(*) c FROM quiz_results", fetchone=True)["c"]
    coding_count = run_query("SELECT COUNT(*) c FROM coding_practice", fetchone=True)["c"]
    interview_count = run_query("SELECT COUNT(*) c FROM mock_interview_results", fetchone=True)["c"]
    resume_count = run_query("SELECT COUNT(*) c FROM resume_data", fetchone=True)["c"]
    goal_count = run_query("SELECT COUNT(*) c FROM goals", fetchone=True)["c"]
    goals_completed = run_query("SELECT COUNT(*) c FROM goals WHERE status='Completed'", fetchone=True)["c"]
    project_count = run_query("SELECT COUNT(*) c FROM projects", fetchone=True)["c"]
    notif_count = run_query("SELECT COUNT(*) c FROM notifications", fetchone=True)["c"]
    by_category = run_query("SELECT category, COUNT(*) c FROM users GROUP BY category", fetch=True)
    ats_rows = run_query("SELECT ats_score FROM resume_data WHERE ats_score > 0", fetch=True)
    avg_ats = round(sum(r["ats_score"] for r in ats_rows) / len(ats_rows), 1) if ats_rows else 0
    top_courses = run_query("""SELECT c.title, COUNT(lp.id) enrollments FROM courses c
                                LEFT JOIN learning_progress lp ON lp.module = c.title
                                GROUP BY c.id ORDER BY enrollments DESC LIMIT 5""", fetch=True)
    top_learners = run_query("""SELECT u.fullname, COUNT(lp.id) tasks, AVG(lp.progress_pct) avg_pct
                                 FROM users u LEFT JOIN learning_progress lp ON u.id = lp.user_id
                                 GROUP BY u.id ORDER BY tasks DESC LIMIT 5""", fetch=True)

    section_title("Report Preview")
    glass_card_open()
    st.write(f"**Total Users:** {total_users}  |  **Active Users:** {active_users}")
    st.write(f"**Total Courses:** {total_courses}  |  **Career Paths:** {total_career_paths}")
    st.write(f"**Certificates Issued:** {total_certs}  |  **Quizzes Completed:** {quiz_count}")
    st.write(f"**Coding Submissions:** {coding_count}  |  **Mock Interviews:** {interview_count}  |  **Resumes Analyzed:** {resume_count} (avg ATS {avg_ats})")
    st.write(f"**Goals Set:** {goal_count} (Completed: {goals_completed})  |  **Projects/Applications Tracked:** {project_count}")
    glass_card_close()
    ...
    if st.button("📄 Generate Full Platform Report PDF", type="primary"):
        sections = {
            "Platform Summary": [
                ...
                f"Total Courses: {total_courses}", f"Career Paths Available: {total_career_paths}",
                f"Certificates Issued: {total_certs}", f"Notifications Sent: {notif_count}",
            ],
            "Users by Category": [f"{r['category']}: {r['c']}" for r in by_category],
            "Feature Engagement Across the Platform": [
                f"Quizzes Completed: {quiz_count}",
                f"Coding Practice Submissions: {coding_count}",
                f"Mock Interviews Conducted: {interview_count}",
                f"Resumes Analyzed (ATS): {resume_count}  |  Average ATS Score: {avg_ats}/100",
                f"Goals Set: {goal_count}  |  Goals Completed: {goals_completed}",
                f"Projects / Internships / Placement Applications Tracked: {project_count}",
            ],
            "Top 5 Most-Enrolled Courses": [f"{c['title']} — {c['enrollments']} enrollments" for c in top_courses] or ["No enrollments yet."],
            "Top 5 Most Active Learners": [f"{l['fullname']} — {l['tasks']} tasks, avg progress {round(l['avg_pct'] or 0,1)}%" for l in top_learners] or ["No learner activity yet."],
        }
        pdf_bytes = generate_report_pdf("Talent Management Platform — Platform Report", admin["fullname"], sections)
